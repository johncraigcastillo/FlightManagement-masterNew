...
const updateFlight = async (req, res) => {
    ...
    try {
        ...
        transactions.push(`UPDATE "Flight"
                           SET date=${date},
                               time=${time},
        ...
        fs.appendFileSync("transactions.sql", transactions.join("\n"), function (err) {
            if (err) throw err;
        });
        // console.log(query);
        // successMessage.count = query.rowCount;
        ...
        fs.appendFileSync("transactions.sql", transactions.join(""), function (err) {
            if (err) throw err;
        });
        return res.status(status.error).send(errorMessage);
    }
...
//todo: create transactions for updateFlightMaintenance and append to transactions.sql
const updateFlightMaintenance = async (req, res) => {
    ...
    try {
        await pool.query("BEGIN;");
        transactions.push("BEGIN;\n");
        const query = await pool.query(sql, [flightID, isFlightReady, hasRefilled, hasCleaned]);
        transactions.push(`UPDATE "Flight Maintenance"
                           SET is_flight_ready=${isFlightReady},
                               has_refilled=${hasRefilled},
